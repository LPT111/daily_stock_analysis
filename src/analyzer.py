...
## 📰 舆情情报
"""
        dual_source_validation = context.get("dual_source_validation") or {}
        if dual_source_validation:
            prompt += f"""
## 行情与估值双源校验（最高优先级事实约束）
以下字段由腾讯财经与东方财富交叉校验：
```json
{json.dumps(dual_source_validation, ensure_ascii=False, indent=2)}
```
- 只有 `status=confirmed` 的字段可以进入事实、评分和决策依据。
- 标记为“待核实”的价格、涨跌幅、PE、PB 必须忽略，不得据此给出买卖建议。
- 动态 PE 小于或等于 0 代表当前口径下盈利为负，不能解读为“几千倍高估值”或“估值泡沫”。
- 若报告内其他行情字段与双源校验冲突，以本区块为准并明确提示数据冲突。
"""
        if news_context:
            prompt += f"""
...
5. 🔗 **来源规则（强制）**：
   - 每条被采用的新闻必须同时包含：标题、发布时间、来源名称、原文 URL
   - 缺少任一项都不得进入 `latest_news`、`risk_alerts` 或 `positive_catalysts`
   - 不得把新闻与股价同日出现直接写成因果；除非原文明确说明，否则禁止推测“政策导致”“限制引发”“资金因消息出逃”
...
- **三个月结构**：必须综合最近约 3 个月 K 线，不得仅凭单日涨跌或单根 K 线下结论
- **直接动作**：面向用户的核心动作只能是“买入 / 持有 / 减仓 / 卖出”之一；如使用 `decision_type=sell` 表达部分退出，正文必须明确写“减仓”而不是“卖出”
- **保守风格**：必须给出买入区间、建议仓位、止损线、结论失效条件；没有可靠入场条件时买入区间写“暂不设置”
- **无持仓假设**：不要索要或假设用户成本价、持仓数量和最大亏损；同时分别给出空仓者与持仓者的条件化建议
- **卖出纪律**：不得仅凭单个未经双源确认的估值字段、单条新闻或单日跌幅给出清仓结论；减仓/卖出须有三个月结构破坏与可核验风险的共同证据
- **检查清单**：每项用 ✅/⚠️/❌ 标记
