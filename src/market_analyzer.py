...
（列出需要关注的风险点；最后补充“建议仅供参考，不构成投资建议”。）"""
        ...
        add_section(
            "消息催化",
            "（只引用同时具备标题、发布时间、来源和原文链接的近三日新闻；不得把新闻与指数涨跌写成未经来源支持的因果，不要推断未提供的政策、资金流、市场宽度或板块榜）",
        )
...
- “消息催化”中每条新闻必须带标题、发布时间、来源和原文链接，缺少任一项则忽略
- 禁止把同期新闻与指数或板块涨跌直接写成因果；只有原文明确说明时才能陈述因果
- 禁止自行补写“政策导致”“外部限制引发”“资金因消息出逃”等未被来源支持的解释
{data_boundary_requirement}
